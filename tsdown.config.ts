 ...
 * - lib/client.js — the browser bundle: a closure-factory artifact that calls
 *   `window.__ModuleLoader__.load({id, factory})` and resolves externals
 *   through the injected require (the DSH loader module table — cordis DI
...
/**
 * The browser platform modules the DSH shell shares into its frozen module
 * table; the client bundle leaves them external (they are resolved through
 * the loader's require, never bundled).
 */
...
/** Node half: the host loader imports these from lib/. */
...
/** Browser half: the plugin bundle served from /plugins/<id>/client.js. */
